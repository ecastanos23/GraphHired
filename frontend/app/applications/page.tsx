 ...
 * Página de Postulaciones y Citas. Partes: seleccion de candidato, postulaciones con citas y timeline de agentes.
 */
'use client';
...
function ApplicationsContent() {
  ...
  const fetchProcess = async (candidateId: number) => {
    ...
    try {
      ...
      console.log(`Cargando trazabilidad de agentes para candidato ${candidateId}`);
      const events = await get<AgentEvent[]>(`/api/agents/trace?candidate_id=${candidateId}&limit=50`);
      setTrace(events || []);
      console.log(`Trazabilidad de agentes cargada:`, events?.length || 0);
    } catch (err) {
      // La trazabilidad es informativa solamente - no es crítico si falla
      console.warn('No se pudo cargar la trazabilidad de agentes (optional):', err);
    }
  };
    ...
    <div className="space-y-8">
        ...
        <div className="bg-[linear-gradient(135deg,_#0f172a_0%,_#111827_50%,_#064e3b_100%)] px-6 py-8 text-white sm:px-8">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl space-y-4">
              <p className="text-xs font-semibold uppercase tracking-[0.28em] text-emerald-100">Postulaciones</p>
              <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">
                Tus postulaciones y citas
              </h1>
              <p className="text-sm leading-6 text-slate-200 sm:text-base">
                Gestiona todas tus postulaciones, agrega citas y visualiza qué hizo cada agente en el proceso.
              </p>
            </div>
      ...
      {selectedCandidate && loading && (
        <div className="rounded-2xl border border-slate-200 bg-white p-12 text-center shadow-sm">
          ...
          <p className="mt-4 text-slate-600">Cargando postulaciones...</p>
        </div>
      )}
            ...
            <div className="mt-6 space-y-4">
              {applications.length === 0 && (
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-6 text-sm text-slate-600">
                  Todavia no hay postulaciones. Usa el boton Postularme en una vacante recomendada desde el dashboard.
                </div>
              )}
...
export default function ApplicationsPage() {
  return (
    <Suspense fallback={<div className="rounded-2xl border border-slate-200 bg-white p-8 text-center text-slate-600">Cargando postulaciones...</div>}>
      <ApplicationsContent />
    </Suspense>
  );
}
